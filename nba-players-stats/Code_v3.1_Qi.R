...
###convert MVP data to numerical type

t.d$MVP <- ifelse(t.d$MVP=='yes', 1,0)
v.d$MVP <- ifelse(v.d$MVP=='yes', 1,0)

prime_model <- glm(MVP ~., data = t.d, family = "binomial")
options(scipen=999)
summary(logit.reg)

### generate confusion matrix for training data
pred_train<-predict(logit.reg,t.d,type = "response")
confusionMatrix(as.factor(ifelse(pred_train>0.7,1,0)),as.factor(t.d$MVP))

### repeat the code for the validation set
pred_valid<-predict(logit.reg,v.d,type = "response")
confusionMatrix(as.factor(ifelse(pred_valid>0.5,1,0)),as.factor(v.d$MVP)) #Balanced Accuracy : 0.7238
confusionMatrix(as.factor(ifelse(pred_valid>0.7,1,0)),as.factor(v.d$MVP)) #Balanced Accuracy : 0.6032
confusionMatrix(as.factor(ifelse(pred_valid>0.9,1,0)),as.factor(v.d$MVP)) #Balanced Accuracy : 0.5172

library(pROC)

r<-roc(t.d$MVP,pred_train)#ROC curve for traning dataset
plot.roc(r)

r<-roc(v.d$MVP,pred_valid)#ROC curve for validation dataset
plot.roc(r)
