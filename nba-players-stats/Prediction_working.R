...
library(ggplot2)
season_stats$PPG <- as.matrix(season_stats$PTS / season_stats$G )
season_stats$PPG
...
max(MVPTable_s$MVP_Year)
max(season_stats_2$Year)
...
###########
#edited by abhishek
#changing df.set as it was incorrectly complted
df.set<-df.set[,-c(21)]

##########
#modeling
# tran.index<-sample(c(1:dim(df)[1]), dim(df)[1]*0.5) #pick 50% of the data to train
# train.dataset<-df.set[tran.index,]
# validation.dataset<-df.set[-tran.index,]#


#####################
#hit and try llatest attempt to reduce the sample data
df_1<-df.set[,-c(1,3,5:14,19,20)]
tran.index_1<-sample(c(1:dim(df)[1]), dim(df)[1]*0.5)
train.dataset_1<-df_1[tran.index_1,]
validation.dataset_1<-df_1[-tran.index_1,]
...
library(caret)
# ##decision tree
# default.ct <- rpart(MVP ~ ., data = train.dataset, method = "class")
# prp(default.ct)
# 
...
# 
# # custome tree
# cust.ct <- rpart(MVP ~ ., data = train.dataset, method = "class", maxdepth = 2)
# prp(cust.ct)
# 
...
# model_set_2
# prediction<-predict(model_set_2,train.dataset)
# 
# table(prediction)
# 
# predict_validation<-predict(model_set_2,validation.dataset)
# table(predict_validation)

###############################working till here#########
summary(df_1)
set.seed(1)
model_set_3<-train(MVP ~Age+GS+STL+BLK+TOV+PTS,data = train.dataset_1,method="glm",preProcess=c('scale','center'))
pred_12<-predict(model_set_3,train.dataset_1)
table(pred_12)
# confusionMatrix(pred_12,as.factor(train.dataset_1$MVP))
pred12_validate<-predict(model_set_3,validation.dataset_1)
table(pred12_validate)
...
# print(msg)
# }
# predict_func(deeper.ct,1,1,1,1,1,1)

###########function 2
summary(df_1)
pred_test_1<-predict(model_set_3,df_1)
 summary(pred_test)
predict_func<-function(m,Age,GS,STL,BLK,TOV,PTS)
  {
  new1<-predict(m,data.frame(Age=Age,GS=GS,STL=STL,BLK=BLK,TOV=TOV,PTS=PTS))
msg<-paste("Age=",Age,"GS=",GS,"STL=",STL,"BLK=",BLK,"TOV=",TOV,"PTS=",PTS,"MVP==>",format(new1),sep ="")
print(msg)
}
predict_func(model_set_3,24,82,259,131,250,2868)
str(df_1)
#####################################
######################